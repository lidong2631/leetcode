...
public class Solution {
    ...
    private TreeNode helper(int[] postorder, int postStart, int postEnd, int[] inorder, int inStart, int inEnd, HashMap<Integer, Integer> map) {
        if(postStart>postEnd || inStart>inEnd)
            return null;
        TreeNode root = new TreeNode(postorder[postEnd]);
        int index = map.get(postorder[postEnd]);
        root.left = helper(postorder, postStart, postStart+(postEnd-index), inorder, index+1, inEnd, map);	//主要这两行不同 看一下
        root.right = helper(postorder, postEnd-(postEnd-index), postEnd, inorder, index+1, inEnd, map);		//
        return root;
    }
}
