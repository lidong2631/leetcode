...
=======
>>>>>>> 855d3e42a08dbc7790166f20db2004754988236e
class Solution:
    # @return an integer
    def divide(self, dividend, divisor):
        if divisor == 0:        #除数为0的情况
...
public class Solution {
    public int divide(int dividend, int divisor) {
        if(divisor==0)                 //若除数为0 返回最大整数值
            return Integer.MAX_VALUE;
        int res = 0;
        if(dividend==Integer.MIN_VALUE) {   //若被除数为最小整数值
            if(divisor==-1)                 //如果除数为－1 要特别处理一下 因为leetcode规定答案是2147483647 见下面
                return Integer.MAX_VALUE;
            res = 1;
            dividend+=Math.abs(divisor);    //这里被除数先减去一个除数的值保证之后取绝对值不会越界 同时res要加1
        }
        if(divisor==Integer.MIN_VALUE)  //如果除数也为最小整数 则73行取绝对值会越界但这里会返回结果所以无所谓了 否则一般情况返回0
            return res;
        boolean sign = (dividend^divisor)>>>31==1;  //判断符号的套路 还可以写(dividend<0)^(divisor<0)
        dividend = Math.abs(dividend);
        divisor = Math.abs(divisor);
        int digit = 0;
        while((dividend>>1)>=divisor) { //找小于dividend的最大2的幂
            divisor<<=1;
            digit+=1;
        }
        while(digit>=0) {
            if(dividend>=divisor) { //每次看dividend是否包含divisor 有就将2的幂加到res
                dividend-=divisor;
                res+=(1<<digit);
            }
            divisor>>=1;
...
注意对越界处理最好不要转成long 因为这样并没有真正解决问题 看code ganker评论
