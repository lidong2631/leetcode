class Solution:
    ...
    def climbStairs(self, n):
        dp = [0 for i in range(n+1)]		#初始化dp list
        dp[0] = 1; dp[1] = 1 						#初始值dp[0], dp[1] dp[0]必须设为1 因为后面dp[2]要靠它计算 dp[1]为爬1楼的走法 只有一种走一步
        for i in range(2, n+1):					#从n＝2开始循环 用动态规划
            dp[i] = dp[i-1] + dp[i-2]		#状态方程 走n个台阶走法 先走n－2台阶再走2个台阶 ＋ 先走n－1台阶再走1个台阶
        return dp[n]
