...
public class Solution {
    public int firstMissingPositive(int[] A) {
        ...
        {
            if(A[i]!=i+1)   //这里只能写A[i]!=i+1 如果A[i]是负数用A[A[i]-1]判断数组会越界
                return i+1;
        }
        return A.length+1;
    }
...
注意 这题要看code ganker评论 首先62行那里的A[A[i]-1]!=A[i]那里不可以替换成A[i]-1!=i 因为如果有重复元素会出现死循环 [1,1], [1,2,1]

主要因为A[i]!=i+1判断当前位置的值是不是符合要求 而A[A[i]-1]!=A[i]判断的是指定位置A[A[i]-1]值是不是符合要求
