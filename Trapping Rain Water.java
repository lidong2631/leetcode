public class Solution {
    public int trap(int[] height) {
        ...
        while (left < right) {
            int minHeight = Math.min(height[left], height[right]);      // careful need use minHeight to compare
            if (height[left] == minHeight) {
                left++;
                while (left < right && height[left] <= minHeight) {     // careful height[left] <= height[left-1] wrong!
                    max += minHeight - height[left];
                    left++;
                }
            }
            else {
                right--;
                while (left < right && height[right] <= minHeight) {    // same as above
                    max += minHeight - height[right];
                    right--;
                }
            }
        }
