...
worst case is that for everyHits happened after 5 mins, it will have to traverse the list from beginning to end; 
if LEN is n for every 5 mins, then the operation is linear. For read-heavy request, this is unacceptable.

if huge amount of hits happened at the same timestamp, this solution will takes too much memory since each element in queue is a single hit. 
It's better to map timestamp to the number of hits at this timestamp.

https://leetcode.com/problems/design-hit-counter/discuss/83505/Simple-Java-Solution-with-explanation
